...
import type { CaseStudiesResponse } from "@/lib/api";
      ...
      <div className={`container mx-auto ${SPACING.container} relative z-10`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 max-w-7xl mx-auto">
          {studies.map((study, index) => (
            <Link
              key={`case-study-${study._id ?? index}`}
              href={`/${urlSeg}/case-study/${slugify(study.title)}-${study.order}`}
              ...
              <div className="relative h-44 sm:h-52 md:h-48 lg:h-56 overflow-hidden bg-muted">
                {study.image ? (
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-muted to-muted/50">
                    <span className="text-muted-foreground text-sm font-medium">{study.title || "Case Study"}</span>
                  </div>
                )}
              </div>
