          ...
          {services.map((service, index) => {
            ...
            return (
              <motion.div
                ...
                className="relative bg-card border border-border p-5 sm:p-6 md:p-7 lg:p-9 xl:p-10 rounded-xl sm:rounded-2xl hover:border-primary/40 hover:shadow-[0_20px_60px_-15px_hsl(var(--primary)/0.2)] transition-all duration-700 group overflow-hidden"
                variants={{
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row md:flex-col lg:flex-row items-start gap-4 sm:gap-5 md:gap-5 lg:gap-6 relative z-10">
                  <motion.div
                    className="flex-shrink-0 w-14 h-14 sm:w-16 sm:h-16 md:w-14 md:h-14 lg:w-16 lg:h-16 rounded-lg sm:rounded-xl bg-primary text-primary-foreground flex items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-[0_10px_30px_-10px_hsl(var(--primary)/0.4)]"
                    animate={{ y: [0, -5, 0] }}
                  ...
                  <div className="flex-1 w-full">
                    <h3 className="text-xl sm:text-2xl md:text-xl lg:text-2xl font-bold mb-2 sm:mb-3 md:mb-3 text-foreground group-hover:text-primary transition-colors duration-300">
                      {service.title}
                    </h3>
                    <p className="text-sm sm:text-base md:text-sm lg:text-base text-muted-foreground mb-3 sm:mb-4 md:mb-4 leading-relaxed">
                      {service.description}
                    </p>
                    <div className="inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 md:px-3.5 md:py-1.5 lg:px-4 lg:py-2 bg-primary/10 border border-primary/30 rounded-full text-primary text-xs sm:text-sm md:text-xs lg:text-sm font-semibold group-hover:bg-primary group-hover:text-primary-foreground group-hover:border-primary transition-all duration-500">
